...
def PathFinder(input_num, indices_to_exlcude, polygonal_numbers):
    ...
    if len(indices_to_exlcude) >= len(polygonal_numbers):
        ...
        if len(posts) == 0:
            ...
            for post in posts:
                ret = PathFinder(post[0], indices_to_exlcude + [post[1]], polygonal_numbers)
                if ret is not False:
...
if __name__ == "__main__":
    max_digits = 4
    iterators = [Polygonal(3, max_digits), Polygonal(4, max_digits), Polygonal(5, max_digits), Polygonal(6, max_digits), Polygonal(7, max_digits), Polygonal(8, max_digits)]
    ...
    polygonal_numbers = [[x for x in polys if len(str(x)) == max_digits] for polys in polygonal_numbers]
    ...
    polygonal_numbers = [[(int(str(x)[0:2]), int(str(x)[2:])) for x in polys] for polys in polygonal_numbers]  # easier for looking at digits halves
